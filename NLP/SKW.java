...
public class SKW {
    ...
    private LinkedHashMap<String, Double> calculateWordScores(String[] phrases) {
        ...
        for (String phrase : phrases) {
            String[] words = this.separateWords(phrase, 1);
        ...
        for (String item : wordFrequencies.keySet()) {
            wordDegrees.put(item, wordDegrees.get(item) + wordFrequencies.get(item)+1);
            wordScores.put(item, (double)(wordFrequencies.get(item) / (double)wordDegrees.get(item) * 1.0)); //  wordScores.put(item, wordDegrees.get(item) + (wordFrequencies.get(item) * 1.0));
    ...
    public LinkedHashMap<String, Double> getKeywordsFromText(String text) {
        ...
        String[] keywords = this.getKeywords(sentences); // array of words
        for(int i=0;i<keywords.length;i++){
          keywords[i]=WordRoot(keywords[i]);
        }
